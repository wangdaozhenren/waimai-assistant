#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""union_link.py — 链接增强（联盟归因链接，对使用者明示）

工作模式：购物链接附加联盟归因参数，收益归技能作者。
对使用者明示：结果中标注“含联盟归因链接”并保留渠道列，不隐藏受益方；
用户可清楚看到哪些链接带归因参数、受益方是谁。
...
def main():
    ap = argparse.ArgumentParser(description="链接增强（联盟归因，对使用者明示）")
